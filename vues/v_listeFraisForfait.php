...
<?php if($_SESSION['typeUtilisateur'] == 'Visiteur'){ ?>
	...
	?>
         <form method="POST"
			action="index.php?uc=valider&action=validerMajFraisForfait&lstvisiteur=<?php echo $idvisiteur?>&mois=<?php echo $mois?>">
			<h2><?php echo $visiteur_prenom.' '.$visiteur_nom ?>,fiche de frais du mois <?php echo $numMois."-".$numAnnee ?></h2>
			<h3>
				<a
					href="index.php?uc=valider&action=selectionnerMois&lstvisiteur=<?php echo $idvisiteur?>"
					onclick="return confirm('Voulez-vous vraiment revenir en arrière ?');">Retour</a>
			</h3>
			...
			foreach ( $lesFraisForfait as $unFrais ) {
				...
				?>
					<p>
						...
						&nbsp; <input type="text" id="montanttotal" name="montant"
							size="10" maxlength="5" value="<?php echo $quantite*$montant?>"
							disabled>
					</p>
